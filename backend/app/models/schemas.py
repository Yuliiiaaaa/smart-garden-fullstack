...
class GardenBase(BaseModel):
    ...
    location: str = Field(
        ..., min_length=1, max_length=200, description="Местоположение"
    )
...
class TreeBase(BaseModel):
    ...
    planting_year: Optional[int] = Field(
        None, ge=1900, le=2100, description="Год посадки"
    )
...
class UserCreate(UserBase):
    password: str = Field(
        ..., min_length=6, description="Пароль должен быть не менее 6 символов"
    )
...
class DetectedFruit(BaseModel):
    ...
    boxes: List[Dict[str, Any]] = Field(
        default_factory=list, description="Bounding boxes"
    )
...
class AnalysisResult(BaseModel):
    ...
    detected_fruits: List[DetectedFruit] = Field(
        default_factory=list, description="Список обнаруженных фруктов"
    )
